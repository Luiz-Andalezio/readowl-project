...
export async function POST(req: Request) {
  ...
  // Lightweight IP rate limit: 5 requests per 15 minutes
  // Prefer Redis-backed limiter when configured; fallback to in-memory
  let headers: Headers;
  let allowed = true;
  try {
  ...
  if (!allowed) {
    // Maintain privacy but enforce backoff
  ...
  if (!email || !isValidEmail(email)) {
    // For privacy, respond 200 even if invalid
  ...
  if (!user) {
    ...
    return new NextResponse(JSON.stringify({ ok: true }), { status: 200, headers }); // privacy
  }

  // Per-user cooldown: if a token was created in the last 120s, silently accept without sending again
  ...
  // Prefer NEXTAUTH_URL; otherwise, derive origin from request to support dev environments
  ...
  try {
    await sendMail({
      ...
      attachments: [
        {
          filename: 'logo.png',
          path: process.cwd() + '/public/img/mascot/logo-white-line.png',
          cid: 'readowl-logo'
        }
      ],
    });
